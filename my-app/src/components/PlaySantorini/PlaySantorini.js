...
const PlaySantorini = () => {
        ...
        return (
            <div className="colourPicker">
                ...
                <div>
                    ...
                    <div className="colourOptions">
                        {playerColour !== "clay" && <div onClick={() => setEnemyColour("clay")}>
                            <Tile defaultColour={"clay"}></Tile>
                        </div>}
                        {playerColour !== "white" && <div onClick={() => setEnemyColour("white")}>
                            <Tile defaultColour={"white"}></Tile>
                        </div>}
                        {playerColour !== "blue" && <div onClick={() => setEnemyColour("blue")}>
                            <Tile defaultColour={"blue"}></Tile>
                        </div>}
                    </div>
                </div>}
        ...
        return (
            <div className="enginePicker">
                ...
                <div className="engineOptions">
                    <div className="engineOption" onClick={() => {setEngine("random"); setStatus("placeFirstBuilder")}} style={{backgroundColor: "rgba(127, 255, 0, .6)"}}><h3 className="bold">Random</h3> <p>CPU moves at random, very easy</p></div>
                    <div className="engineOption" onClick={() => {setEngine("greedy"); setStatus("placeFirstBuilder")}} style={{backgroundColor: "rgba(255, 255, 0, .6)"}}><h3 className="bold">Greedy</h3> <p>CPU moves are shortsighted, easy</p></div>
                    <div className="engineOption" onClick={() => {setEngine("minimax"); setStatus("placeFirstBuilder")}} style={{backgroundColor: "rgba(255, 0, 0, .6)"}}><h3 className="bold">Minimax</h3> <p>CPU plans ahead, hard</p></div>
                </div>
            </div>
        )
    }
        ...
        return (
            <div className="sideDisplay">
                ...
                <div className="sideDivisor">
                    ...
                    {outcome ? <p>Game Over!</p> : <>
                        {status === "gameOptions" && <p>Choosing Game Options</p>}
                        {status === "placeFirstBuilder" && <p>Place your first builder</p>}
                        {status === "placeSecondBuilder" && <p>Place your second builder</p>}
                        {status === "playerSelectBuilder" && <p>Select a builder to move</p>}
                        {status === "playerMoveBuilder" && <p>Select a square to move to (or select other builder)</p>}
                        {status === "playerBuilderBuild" && <p>Select a square to build on</p>}
                ...
                <div className="sideDivisor">
                    <div>
                        ...
                        {engine && <h3 className="bold">{engine.charAt(0).toUpperCase() + engine.slice(1)}</h3>}
                        {engine === "random" && <p>The random engine moves and builds completely randomly</p>}
                        {engine === "greedy" && <p>The greedy engine always moves a piece up, if possible, else does a random move</p>}
                        {engine === "minimax" && <p>The minimax engine looks at all possible game boards three moves ahead, and picks the move according to the best position it can be in after 3 turns</p>}
                    </div>
                </div>
            </div>
        )
    ...
    const Tile = ({id, value, defaultColour}) => {
        ...
        <div className="tile" id={id} onClick={() => handleTileClick(id, value)}>
            ...
            {availableMoveTiles.includes(id) && <div className="availableMove"></div>}
            {availableBuildTiles.includes(id) && tileLevel < 3 && <div className="availableBuild"></div>}
            {availableBuildTiles.includes(id) && tileLevel === 3 && <div className="availableBuildDome"></div>}
            ...
            <div className="gridContainer">
                <div className="grid">
                    {Array(25).fill("").map((_, index) => {
                        return <Tile id={`${Math.floor(index / 5)}-${index % 5}`} key={index} value={grid[Math.floor(index / 5)][index % 5]}/>
                    })}
                </div>
                {outcome && 
                <div className="endGameOverlay">
                    ...
                    {outcome === "playerWins" && (
                        <div className="endGameDisplay">
                            ...
                            <p className="endText">You beat the {engine} engine by standing on a level three tower first.</p>
                            {engine !== "minimax" && <p className="endText">Now see if you can beat the minimax engine!</p>}
                        </div>)}
                    {outcome === "playerBlocked" && (
                        <div className="endGameDisplay">
                            <h2 className="endTitle">You lost!</h2>
                            <p className="endText">You were beat the {engine} engine by not being able to move any builders on your turn.</p>
                        </div>)}
                    {outcome === "cpuWins" && (
                        <div className="endGameDisplay">
                            <h2 className="endTitle">You lost!</h2>
                            <p className="endText">You were beat the {engine} engine who was able to stand on a level three tower first.</p>
                        </div>)}
                    {outcome === "cpuBlocked" && (
                        <div className="endGameDisplay">
                            <h2 className="endTitle">You won!</h2>
                            <p className="endText">You beat the {engine} engine by the cpu not being able to move any builders on their turn.</p>
                            {engine !== "minimax" && <p className="endText">Now see if you can beat the minimax engine!</p>}
                        </div>)}
    ...
    const placeBuilder = (id, value) => {
        ...
        if (value === 0) {
            ...
            setGrid(newGrid)
            status === "placeFirstBuilder" ? setStatus("placeSecondBuilder") : setStatus("cpuPlace")
        }
    }
    ...
    const selectBuilder = (id, value) => {
        if (10 <= value && value <= 13) {
            ...
            const movesObject = Object.values(result).filter(e => e.location[0] === parseInt(row) && e.location[1] === parseInt(col))
            const stringAvailables = movesObject[0].validMoves.map(e => `${e[0]}-${e[1]}`)
    ...
    const moveBuilder = (id) => {
        ...
        if (newGrid[row][col] === 13) {
            ...
            const validBuildsArray = validBuilds([parseInt(row), parseInt(col)], newGrid)
    ...
    <>
        ...
        <div className="explanationText">
            ...
            <p>First, pick your colour and the CPUs colour. Then, pick an engine to play against. This is important as each engine follows a different algorithm with a different playstyle, some more effective than others.</p>
            <p>Now select two different tiles to place your workers on, then the CPU will pick two tiles for their workers.</p>
            <h2 className="bold">On Your Turn</h2>
            <h3 className="bold">Move a builder</h3>
            <p>Select one of your two builders by clicking the tile. Then click an available tile to move the builder to. This must be an unoccupied tile at most one level higher than your current standing. Your builder can drop down any number of levels on their move.</p>
            <h3 className="bold">Build up one tile</h3>
            <p>Your moved builder must now build on one adjacent unoccupied tile. You may build at any height. Building on a level three domes the tile, meaning it cannot now be stood on for the rest of the game</p>
            <h2 className="bold">How To Win</h2>
            <p>The first player to have a builder standing on a level three tile wins the game! You also win the game if your opponent has no valid moves on their turn, in the case they are blocked in. You can find some basic strategy tips <a href="https://jackburgess.me/santorini" target="_blank" rel="noreferrer" className='textLink'>here</a>.</p>
        </div>
    </>)
}
